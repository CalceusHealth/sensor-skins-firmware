...
|---|---|---|---|
| `v2.0.60` | **built — latest (stream, BINARY_V2 v3) — bench-verify vbat scale (SEN-102) before field flash** | Sleep v2 (SEN-95..SEN-104): IMU WoM sleep ~5 µA (was ~0.5 mA); motion/load-gated `;CX` latch (300 s grace/motion/load holds; worn-load = any 1 FSR ch 5 s-median ≥ 25 raw, session-gated); IDLE_CONNECTED 500 ms cadence when connected+idle 120 s; sleep wake = IMU motion or BLE connect only (level-wake deleted); wall-clock idle timers; vbat reseed + protection-wake debounced ×2; **vbat ÷2 ratio-metric + 30 ms settle (QB RAW ~1.58× up)**; protection sleep ejects connections after 30 s unless charging; `;QI` `SESSION=`/`RATE=`. Wire format v3 unchanged (IMU fields 0 while in WoM). App pairing: `;CX` reconcile-on-connect (mobile `5272c79`, `0b4b668`). | `9a38e77` (`tim`) |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| built — latest (sleep v2; bench-verify vbat first) | `v2.0.60` | `lhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_lhs_v2.0.60_sensorskins.zip` | `8aeb45e358044bc48a5c6b1d1f1b18f19cc43120e4ca9c0f2e0d1e00a243b1a9` | `9a38e77` |
| built — latest (sleep v2; bench-verify vbat first) | `v2.0.60` | `rhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_rhs_v2.0.60_sensorskins.zip` | `1a470bdf49ef757966859f2cb7e8f0f0f5725295a07e470adfb0ea5edbb35412` | `9a38e77` |
