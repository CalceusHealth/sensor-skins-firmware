...
- **Latest build: `v2.0.57` (stream / `BINARY_V2`)** — SEN-59 round 3, **the real fix.** Walk-test data proved the steady ~14% "frame loss" / 87.5 Hz ceiling is a **deterministic 1-in-8 skipped sample in the firmware**, not BLE: `device_time_ms` 20 ms gaps are spaced *exactly* every 7th row (754/754) → 7 rows per 80 ms = 87.5 Hz. Cause was `main.c`'s `battery_update_divider` doing cheap battery housekeeping every 8th loop and `continue`-ing past `measure_sensors()`. Fixed to measure on that slot too. The original SEN-59 "BLE transport loss" premise was a misdiagnosis; rounds 1–2 (below) still earn their keep on the secondary contention bursts. Built stream LHS+RHS (`73dc710`); **awaiting walk-test** — expect ~100 Hz on both feet. Do a single-foot control run too.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| built — latest (SEN-59 r3, real fix) | `v2.0.57` | `lhs` | `stream` | `sleep` | `BINARY_V2` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_lhs_v2.0.57_sensorskins.zip` | `4a001f2638f44cf043b8bec9f7dcdc3d4a6dea6aa08ae3bd27d37079ac06704f` | `73dc710` |
| built — latest (SEN-59 r3, real fix) | `v2.0.57` | `rhs` | `stream` | `sleep` | `BINARY_V2` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_rhs_v2.0.57_sensorskins.zip` | `f10ef836889dae5bca9b5b4317404d5c125a66a09e03197542d2de920d3e9029` | `73dc710` |
