...
- **Latest build: `v2.0.61` (stream / `BINARY_V2` v3) — SEN-154 SoftDevice S112 → S132 6.1.1 for BLE Data Length Extension.** S112 has no DLE at all (LL feature probe → `0x1A`), so every ~170 B stream notification cost ~7× 27 B radio packets — the scheduling pressure behind dual-device 100 Hz frame loss on every host platform. Same SDK 15.3.0 / BLE API v6; app code unchanged; wire format v3 unchanged. `NRF_SDH_BLE_GAP_EVENT_LENGTH` 6→12. **DFU path: a device on ≤ v2.0.60 (S112) must first take the one-shot `migrate_s132_*` zip (SoftDevice + rebuilt bootloader + app, sd-req accepts S112 and S132 so retry is safe); the plain `*_v2.0.61` app-only zips are sd-req 0xB7 and only install after migration.** Migration zip bootloader-version = 2 — must exceed the fielded bootloader's settings-page version; if the DFU is rejected on version grounds, rebuild the package with a higher value (`tools/package_migration_dfu.sh <sdk> <side> <bl_version>`). ⚠️ An interrupted SD-swap DFU is the one genuinely bricky failure mode — prove on a test unit before fleet rollout. Verify DLE post-update: btmon shows one ~170–250 B ACL packet per notification (was 7 × 27 B); then dual-device `;CF 100` ≥ 99 Hz/side over 10 min on a fresh connect (clinic-app `tools/dual_spike.py`). Inherits the v2.0.60 vbat-scale caveat. Built all 4 variants + 4 migration zips (`39470b8`).
- **Prior build: `v2.0.60` (stream / `BINARY_V2` v3) — sleep v2 power management (SEN-95..SEN-104).** IMU wake-on-motion sleep (gyro off, accel LP ~5 µA — was ~0.5 mA forever, the deep-discharge kill chain), motion/load-gated `;CX` session latch (forgotten sessions sleep; seated wearers protected by the field-tuned worn-load guard), new **IDLE_CONNECTED** (connected + idle 120 s → 500 ms internal cadence + IMU LP; snaps back on session/motion/deltas/;CF), sleep wake = motion or BLE only (FSR level-wake deleted — fixes in-shoe-unworn oscillation), wall-clock idle timers, vbat reseed/wake debounce (2 samples), **ratio-metric vbat scale (÷2 + 30 ms settle — ⚠️ multimeter-verify ≥3 units before field flash; QB RAW steps ~1.58×, re-baseline the σ≈4 health figure)**, protection sleep ejects connections after 30 s (unless charging), `;QI` gains `SESSION=`/`RATE=`. Stream v3 wire format unchanged; IMU row fields read 0 in IDLE_CONNECTED/sleep. Pairs with app-side `;CX` reconcile-on-connect (mobile `5272c79`/`0b4b668`). Also: legacy flash-summary path compiled out of stream builds (76e1621) and **SEN-106 firmware UVLO** — System OFF (~0.3 uA) below 3100 mV x2 samples in protection sleep, wake via charger PG sense (a flat unit looks dead until placed on the puck). Built stream LHS+RHS (`458a3dc`). Design doc: `SLEEP_LOGIC_V2_PROPOSAL.md`.
- **Prior build: `v2.0.59` (stream / `BINARY_V2` v3) — battery reliability.** Single %-curve (dropped the dual charge/discharge switch that made the reported level jump on plug/PG flicker) + a new low-rate **battery stream frame (frame_type 3)** carrying averaged vbat for real in-session drain metrics (per-row vbat was frozen before because QB can't be polled during native streaming). Additive to the v3 stream. Includes all SEN-68 IMU + SEN-59 work. Needs the v3 decoder (desktop SEN-89 handles frame_type 3; mobile SEN-90). Built stream LHS+RHS (`d698fb6`).
- **Prior build: `v2.0.58` (stream / `BINARY_V2` v3) — SEN-68 IMU in the stream, always-on.** Stream VERSION 2→3: sensor row 74→76 B (temp moved out to a new frame_type 2; `acc[3]`+`gyro[3]` int16 appended; still 3 rows/frame). LSM6DSM read every frame at 208 Hz ODR. ⚠️ **Breaking wire change — needs a v3 host decoder (SEN-90 mobile / SEN-89 desktop / SEN-91 web) before it can be recorded; do not flash for real sessions until then** (the current app would misparse v3). Built stream LHS+RHS (`5ae0931`). Includes the SEN-59 v2.0.57 divider fix (true single-foot 100 Hz); dual-foot contention is tracked in SEN-92 (mobile).
- **Prior build: `v2.0.57` (stream / `BINARY_V2`) — SEN-59 round 3, the real fix, TESTED.** The 87.5 Hz ceiling was a **deterministic 1-in-8 skipped sample** (`main.c` `battery_update_divider` `continue`-ing past `measure_sensors()`), not BLE. Fixed. **Walk-test (sessions `876668b7`/`d0f3e651`) confirms: the every-7th-row 20 ms pattern is GONE and a foot hits true ~100 Hz (LEFT 99.5/99.9 Hz, 100% at 10 ms, 0 gaps).** BUT removing the skip doubled the real per-foot BLE load and exposed the genuine ceiling: **the Pixel 7a can't serve two `CONNECTION_PRIORITY_HIGH` links — it starves the second-connected foot** (RIGHT collapsed to ~57–62 Hz with 400–750 big 100–190 ms gaps; LEFT connects ~3 ms first and wins the airtime; no reconnects; RIGHT was the *clean* foot on v2.0.55, so not a bad unit). **Divider = firmware SEN-59 done; simultaneous dual-foot 100 Hz is a new central-side/mobile problem** (dual-connection scheduling; relates to SEN-66/SEN-69). Firmware idea to try: relax PPCP to ~15 ms so two links interleave.
...
|---|---|---|---|
| `v2.0.61` | **built — latest (stream, BINARY_V2 v3) — DFU via `migrate_s132_*` zip first (SD swap); bench-verify vbat scale (SEN-102) before field flash** | SEN-154: SoftDevice S112 → S132 6.1.1 for Data Length Extension (S112 has no DLE; 0x1A on LL probe). One ~170 B notification becomes one LL packet instead of ~7×27 B — removes the dual-device 100 Hz scheduling pressure at the source, all host platforms. Same SDK/API; app code + v3 wire format unchanged. GAP event length 6→12 (7.5→15 ms). App flash 0x26000, RAM 0x20004000. Bootloader rebuilt against S132 (`secure_bootloader_ble_s132_calceus`, version 2). App-only zips now sd-req `0xB7`; `migrate_s132_*` zips carry SD+BL+app with sd-req `0xB8,0xB7` (retry-safe). Verify: btmon DLE packets, then dual `;CF 100` ≥99 Hz/side 10 min fresh-connect (clinic app `tools/dual_spike.py`). | `39470b8` (`tim`) |
| `v2.0.60` | superseded by `v2.0.61` (S132/DLE) — vbat-scale bench check still open (SEN-102) | Sleep v2 (SEN-95..SEN-104): IMU WoM sleep ~5 µA (was ~0.5 mA); motion/load-gated `;CX` latch (300 s grace/motion/load holds; worn-load = any 1 FSR ch 5 s-median ≥ 25 raw, session-gated); IDLE_CONNECTED 500 ms cadence when connected+idle 120 s; sleep wake = IMU motion or BLE connect only (level-wake deleted); wall-clock idle timers; vbat reseed + protection-wake debounced ×2; **vbat ÷2 ratio-metric + 30 ms settle (QB RAW ~1.58× up)**; protection sleep ejects connections after 30 s unless charging; `;QI` `SESSION=`/`RATE=`. Wire format v3 unchanged (IMU fields 0 while in WoM). Summary path compiled out of stream builds; SEN-106 firmware UVLO (System OFF < 3100 mV, wake on charger PG). App pairing: `;CX` reconcile-on-connect (mobile `5272c79`, `0b4b668`). | `458a3dc` (`tim`) |
| `v2.0.59` | superseded by `v2.0.60` (stream, BINARY_V2 v3) | Battery reliability. **Single curve**: `battery_pack_charge_from_mv()` always uses the resting/discharge table (dropped the `batv_chg_*` charge-curve branch + the monotonic-clamp logic) — the undebounced dual-curve switch on PG/CHG was the main cause of the erratic reported level on/off charge. Charge state still via `battery_current_state()`. New **battery stream frame (frame_type 3, ~1 Hz)**: averaged `vbat_mv` (drain metric) + fresh `vbat_raw` + single-curve `pct` + state char — streamed because QB can't be polled during native recording (why per-row vbat was frozen in field CSVs). Stream stays v3 (additive frame). Desktop decoder (SEN-89) handles it; mobile SEN-90 must too. Also carries all SEN-68 IMU-v3 + SEN-59 fixes. | `d698fb6` (`tim`) |
| `v2.0.58` | superseded by `v2.0.59` (stream, BINARY_V2 v3) — needs v3 decoder | SEN-68: IMU in the binary stream, always-on. Stream **VERSION 2→3**. Row 74→76 B (drop `temp[5]`, append `acc[3]`+`gyro[3]` int16; still 3 rows/frame @ MTU 247). New **frame_type 2** carries `temp[5]` at ~1 Hz. `FLAG_IMU 0x04`. `measure_sensors()` calls `lsm6dsm_update()` each frame (~0.5 ms); LSM6DSM **ODR 52→208 Hz** (was mislabelled 208 in the driver) so 100 Hz reads get fresh samples. ⚠️ **Breaking wire change** — hosts must branch on the version byte; **do NOT flash for real recordings until the mobile decoder (SEN-90) handles v3** or data misparses. Client decoders: SEN-90/89/91. | `5ae0931` (`tim`) |
| `v2.0.57` | tested ✅ divider fixed (single-foot true 100 Hz); dual-foot contention → SEN-92 | SEN-59 round 3 — **the real fix.** Walk-test data (sessions `49dbd950` v2.0.55, `813daad9` v2.0.56) proved the steady ~14% "loss" is a **deterministic 1-in-8 skipped sample**, not BLE: 20 ms device-clock gaps spaced *exactly* every 7th row (754/754) = 7 rows/80 ms = **87.5 Hz** — the ceiling on every foot in every build. Cause: `main.c` `battery_update_divider` did cheap battery housekeeping every 8th loop then `continue`d, skipping `measure_sensors()`. Fix: fall through and measure that slot too (keep `continue` only on the sleep path). The original "BLE transport frame loss" premise was a **misdiagnosis**. Rounds 1–2 retained (they cut the *secondary* bursty contention gaps — v2.0.56 clean foot: 1 big gap vs 213 on v2.0.55). Expect ~100 Hz. | `73dc710` (`tim`) |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| **built — latest; migration package (SD S132 + BL v2 + app; one-shot for S112 devices)** | `v2.0.61` | `lhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/migrate_s132_stream_sleep_lhs_v2.0.61_sensorskins.zip` | `bee21ffee79731a4fedcd61f80a5ed3396f02b72cd528487c7a06434b3f61ce6` | `39470b8` |
| **built — latest; migration package (SD S132 + BL v2 + app; one-shot for S112 devices)** | `v2.0.61` | `rhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/migrate_s132_stream_sleep_rhs_v2.0.61_sensorskins.zip` | `9fa4018ff073b7620a6e58421c25b3a5a94c4394d5f397d0b7e0492d736fa5ed` | `39470b8` |
| built — latest; migration package (nostream variant) | `v2.0.61` | `lhs` | `nostream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/migrate_s132_nostream_sleep_lhs_v2.0.61_sensorskins.zip` | `99ca18f195bd20c0a7d9c2c9ad3c684aa9f02a5b1d5e3ccbd1659782d250f38d` | `39470b8` |
| built — latest; migration package (nostream variant) | `v2.0.61` | `rhs` | `nostream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/migrate_s132_nostream_sleep_rhs_v2.0.61_sensorskins.zip` | `d677dd5ddbe8a9ba6a4a29803a4f9d7762d3946b22527cce168d423894e1bc80` | `39470b8` |
| built — latest; app-only, sd-req 0xB7 (installs only AFTER S132 migration) | `v2.0.61` | `lhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_lhs_v2.0.61_sensorskins.zip` | `d7c5aaf594a5a0cce1909d478be0ef04c2fde71d2b14bbc6a50eeab2ab2f3d3e` | `39470b8` |
| built — latest; app-only, sd-req 0xB7 (installs only AFTER S132 migration) | `v2.0.61` | `rhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_rhs_v2.0.61_sensorskins.zip` | `6412b0b76a01cfc81c915acb49ed6b86d5538f09c7436e6d4244ca736b332d8f` | `39470b8` |
| built — latest; app-only, sd-req 0xB7 (installs only AFTER S132 migration) | `v2.0.61` | `lhs` | `nostream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/nostream_sleep_lhs_v2.0.61_sensorskins.zip` | `4a3683b57211250a32a0f31c283bb54b9112f5b27763a4fd528541eafaedeea8` | `39470b8` |
| built — latest; app-only, sd-req 0xB7 (installs only AFTER S132 migration) | `v2.0.61` | `rhs` | `nostream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/nostream_sleep_rhs_v2.0.61_sensorskins.zip` | `38b669139423d00021ee761f20f2a94f1dae0b54eaabf84f7a1c8608538761c6` | `39470b8` |
| superseded by v2.0.61 (S132/DLE; S112 sd-req 0xB8) | `v2.0.60` | `lhs` | `nostream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/nostream_sleep_lhs_v2.0.60_sensorskins.zip` | `71475d9be9ea0910d550f187d73b3201ffcd520c4a11924a4bc8752591e20235` | `458a3dc` |
| superseded by v2.0.61 (S132/DLE; S112 sd-req 0xB8) | `v2.0.60` | `rhs` | `nostream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/nostream_sleep_rhs_v2.0.60_sensorskins.zip` | `78852caadfb96e9a40206b47711e691a3d0bd7ca7014e1a2b71bad52b95ad8a0` | `458a3dc` |
| superseded by v2.0.61 (S132/DLE; S112 sd-req 0xB8) | `v2.0.60` | `lhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_lhs_v2.0.60_sensorskins.zip` | `725b15bc645d0f0c69b5d454e8c7508a86c30ef4b6227d7956a848798602326d` | `458a3dc` |
| superseded by v2.0.61 (S132/DLE; S112 sd-req 0xB8) | `v2.0.60` | `rhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_rhs_v2.0.60_sensorskins.zip` | `14cf8bccd5a6ffca1beaaffbef490be356721517e23d63abc180a5542bf61c7d` | `458a3dc` |
...
| superseded (IMU v3 + battery) | `v2.0.59` | `rhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_rhs_v2.0.59_sensorskins.zip` | `0c6d895c467a0cc4f7573433317abc9f54be7b505467d00496cc0646041e4636` | `d698fb6` |
| superseded by v2.0.59 (IMU v3) | `v2.0.58` | `lhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_lhs_v2.0.58_sensorskins.zip` | `fd3c7c679d403881a8c5e6ea9aacc111b32d6999f94c60ff1bf9dce5d4346775` | `5ae0931` |
| superseded by v2.0.59 (IMU v3) | `v2.0.58` | `rhs` | `stream` | `sleep` | `BINARY_V2 v3` | `100Hz`* | `GAIN1_2` | `BINARY_V2` | `artefacts/out/stream_sleep_rhs_v2.0.58_sensorskins.zip` | `07fb882d1b97324e7391a7282a8dc97311e174324f807d35169b262fe9936ada` | `5ae0931` |
