...
volatile uint16_t		msg_tx_buffer_end;
		...
		if (msg_rx_cq == MSG_TYPE_QUERY)
		{
			switch (msg_rx_type)
			{
				case MSG_QUERY_SYSINFO:
				{
					...
					msg_add_delim();
					msg_tx_buffer_end += snprintf((uint8_t*)msg_tx_buffer+msg_tx_buffer_end,MSG_TX_BUFFER_SIZE-msg_tx_buffer_end,"UID=%08X,VER=%08X",flash_sysdata.device_uid,flash_sysdata.device_version);
					msg_add_endline();
					ble_reid_tx((uint8_t*) msg_tx_buffer, msg_tx_buffer_end);
					break;
				}
				...
				case MSG_QUERY_BATTLEVEL:
				{
					...
					msg_add_delim();
					msg_tx_buffer_end += snprintf((uint8_t*)(msg_tx_buffer+msg_tx_buffer_end),MSG_TX_BUFFER_SIZE-msg_tx_buffer_end,"%d",ble_data.vdd_mv);
					msg_add_endline();
					ble_reid_tx((uint8_t*) msg_tx_buffer, msg_tx_buffer_end);
					break;
				}
