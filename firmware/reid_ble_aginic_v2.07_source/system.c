...
{
	// Run WDT during CPU sleep (catches frozen WFE loops), pause during debug halt
	NRF_WDT->CONFIG = (WDT_CONFIG_HALT_Pause << WDT_CONFIG_HALT_Pos) |
	                  (WDT_CONFIG_SLEEP_Run  << WDT_CONFIG_SLEEP_Pos);
	NRF_WDT->CRV    = 60 * 32768 - 1; // 60-second timeout at 32.768 kHz LFCLK
...
{
	NRF_WDT->RR[0] = WDT_RR_RR_Reload; // 0x6E524635
}
