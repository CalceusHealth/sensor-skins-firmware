...
#define VDIV_VBAT_R1	(1)
#define VDIV_VBAT_R2	(1)
...
uint16_t adc_read_vbat_mv(void)
{
    ...
    //nrf_gpio_pin_set(PIN_SAMPLE_VBAT);
	//system_wait_for_ms(150);
    ...
    //nrf_gpio_pin_clear(PIN_SAMPLE_VBAT);
    if (ret_err == NRFX_SUCCESS) return (uint16_t) (((reading) * ADC_SCALING_FACTOR * (600/8) * (VDIV_VBAT_R1 + VDIV_VBAT_R2))/(ADC_TOP / 8)/VDIV_VBAT_R2);
	else return 0;
}
